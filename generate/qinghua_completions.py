...
def init_game_plot():
    response = zhipuai.model_api.sse_invoke(
        ...
        prompt="1、我希望你扮演一个基于文本的冒险游戏（游戏主题：王子拯救白雪公主的故事，主角：勇者人杰）；"
               "2、游戏总共8回合，你将回复故事情节内容描述及5个选项。您将回复故事情节内容的描述；"
               "3、你需要首先给我第一个场景及情节描述，并给我提供5个选项；"
               "4、5个选项的前3项是与故事情节内容相关的选项，第4个选项是'换一换',第5个选项是自定义；"
               "5、如果我回复选项前3个中的其中一个（选项序号），则根据选项继续生成下一回合内容；"
               "6、如果我回复换“换一换”，则重新生成当前回合的故事情节内容描述及选项；"
               "7、如果我自定义，我会把自定义的选项内容也一起发送给你，你根据自定义的内容生成下一回合内容；"
               "8、每个回合的故事情节必须控制在80字以内。故事需要在八个回合内结束；"
               "9、故事的8个情节结构分别是故事的开端、情节推进、矛盾产生、关键决策、情节发展、高潮冲突、结局逼近、最终结局，8个情节按此顺序逐层递进；"
               "10、你给我的情节需要有趣好玩，适合儿童阅读，前后逻辑有联系；"
               "11、你返回给我的内容包含如下：1）round：是整数，从1开始递增，如果“换一换”则保持不变；2）chapter：枚举值：分别是故事的开端、"
               "情节推进、矛盾产生、关键决策、情节发展、高潮冲突、结局逼近、最终结局；3）content：具体的故事情节描述内容；"
               "4）choice： 故事对应的3个选项，用a、b、c英文字母序号开头（故事的最后一个回合最后一话不提供选项选择）"
               "12、返回的内容封装成json的格式，健值对参考上述序号9的内容（只需要json格式返回，其他格式内容不需要返回）"
               "13、注意，请严格按照下面示例的格式，内容请按照上述1-10点要求创作，示例："
               "{\"round\":1,\"chapter\":\"故事的开端\",\"content\":\"你收到了霍格沃茨的入学通知书，"
               "搭乘霍格沃茨特快列车来到了这所神秘的魔法学校。列车穿过山峦和森林，最后抵达了你的目的地。远远望去，霍格沃茨魔法学校屹立在一座高高的山顶上，"
               "城堡式的建筑气势恢宏，令人叹为观止。你怀揣着激动的心情，跟随其他新生一起走下列车，准备进入学校开始新的魔法生活。\",\"choice\""
               ":[\"a. 走进学校大门\",\"b. 先去参观大礼堂\",\"c. 沿着湖边小路走\"，\"d. 换一换\"，\"e. 自定义\"]}",
        temperature=1,
        top_p=0.7,
        incremental=True
    )
